<template>
	<view style="overflow-x: hidden;">
      ...
      <view class="vipbox">
        <view class="vip-item" 
          v-for="(item, index) in aSeltVipList" :key="index"
          :class="[nowSeltVip == item ? 'active' : '']"
    ...
    <template v-if="userInfo.group_id < 1">
      <view class="update">
        <u-button type="warning" shape="circle" ripple :custom-style="{color: '#333'}"
          @click="upStar">立即开通 ¥{{ info.group ? (info.group.needmoney * nowSeltVip).toFixed(2) : '' }}</u-button>
      </view>
      <text class="tip" @tap="showWriteCode = true">使用激活码兑换</text>
    </template>
    <template v-else>
      <view class="update" v-if="hasLogin">
        <u-button type="warning" shape="circle" ripple 
    ...
    <u-modal 
      ...
      title="激活码激活" ref="uModal" 
      show-cancel-button confirm-color="#e1aa1b"
      :async-close="true" 
