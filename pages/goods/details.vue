<template>
    ...
    <template v-if="!hasLogin || userInfo.group_id < 1 && Boolean(info.price - info.vip_price)">
      <view class="join-vip">
        <u-icon name="/static/icon/index_icon_rank_0.png" size="28" label="vip会员" 
    ...
    <view class="shop-card">
      <view class="shopcard-info">
        <u-cell-group :border="false">
          <u-cell-item 
            :icon="info.merchant.mer_avatar"
            icon-size="80"
            :title="info.merchant.mer_name" 
            value="进店逛逛"
            center
            padding="20rpx 0"
            @click="toRoute({
    ...
    <view class="info-content">
      ...
      <view class="ic-comment">
        ...
        <view class="ic-momre">
          <u-button type="default" ripple shape="circle" size="mini" 
    ...
    <view class="info-content">
      ...
      <view class="goods-inbox">
        <view class="goods-initem" v-for="(item, index) in goodLikeList" :key="index" 
          @click="toRoute({
    ...
    <view class="navigation-bar">
      <view class="navigation">
        <view class="left">
          <button class="item btn-item-share"
            @click="onStar">
            <u-icon :name="!info.isRelation ? 'star' : 'star-fill'" :size="40" 
          ...
          <template v-if="couponList.length">
            <view class="cliitems" v-for="(item, index) in couponList" :key="index">
              <coupon-item
                ...
                :rule-text="`领取后${item.coupon_time}天内可用`"
                :title="item.coupon_title"
                :type-text="typeText[item.type]"
                :btn-text="item.issue ? '已领取' : '立即领取'"
    ...
    <u-popup v-model="atShowShare" mode="bottom" border-radius="30">
      <view class="share-box">
        ...
        <view class="share-cancel" @click="atShowShare = false">
          <text>取消</text>
        </view>
      </view>
    </u-popup>
...
<script>
    ...
    onShareAppMessage (res) {
      return {
        title: '分享了一个商品',
        path: '/pages/goods/details?id=' + this.id + '&uid=' + (this.userInfo.uid || '')
      }
    },
    methods: {
      // 保存图片
